...
// hall of fame page
    ...
    function splitStr(string memory strIn, string memory delimiter) public pure returns(string[] memory) {
        bytes memory byteStr = bytes(strIn);
        bytes memory delm = bytes(delimiter);
        uint s=0;
        uint i; uint count=0;
        for(i=s; i<byteStr.length; i++){
            uint j;
            for(j=0; j<delm.length; j++){
                if(byteStr[i+j] != delm[j]) break;
            }
            if(j==delm.length){
                i += delm.length;
                s = i;
        ...
        string[] memory resArr = new string[](count+1);
        s=count=0;
        for(i=s; i<byteStr.length; i++){
            uint j;
            for(j=0; j<delm.length; j++){
            ...
            if(j==delm.length){
                string memory strNode = new string(i-s);
                bytes memory byteNode = bytes(strNode);
                for(uint k=s; k<i; k++)
                    byteNode[k-s] = byteStr[k];
                resArr[count++] = string(byteNode);
                i += delm.length;
                s = i;
            }
        }
        if(s>0 && s<byteStr.length-1){
            string memory strNode = new string(byteStr.length-s);
            bytes memory byteNode = bytes(strNode);
            for(uint k=s; k<byteStr.length; k++)
                byteNode[k-s] = byteStr[k];
            resArr[count] = string(byteNode);
        }
        return resArr;
